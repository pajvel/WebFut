...
export async function ownGoal(matchId: number, team: "A" | "B", segment_id?: number) {
  return apiFetch(`/matches/${matchId}/events/own-goal`, {
    method: "POST",
    body: JSON.stringify({ team, segment_id })
  });
}
...
export async function adminPatchSegment(
  matchId: number,
  segmentId: number,
  payload: { score_a?: number; score_b?: number; ended_at?: string; is_butt_game?: boolean }
) {
