...
def _normalize_team_payload(data: dict) -> dict | None:
    ...
    team_a = teams.get("A") or teams.get("team_a") or data.get("A") or data.get("team_a")
    team_b = teams.get("B") or teams.get("team_b") or data.get("B") or data.get("team_b")
...
def _power_metrics(state, teams: dict, venue: str) -> dict:
    team_a = teams.get("A", [])
    team_b = teams.get("B", [])
    eval_split = evaluate_split(state, team_a, team_b, venue)
...
def generate(match_id: int):
    ...
    if len(participants) < 2:
        # Allow single-player lobby to generate a trivial variant.
                ...
                "variants": [
                    {
                        "variant_no": 1,
                        "is_recommended": True,
                        "teams": {"A": participants, "B": []},
                        "why_text": None,
                        "power": _power_metrics(state, {"A": participants, "B": []}, match.venue),
                    }
                ]
            }
        )
            ...
            "variants": [
                {
                    ...
                    .one()
                    .why_text,
                    "power": _power_metrics(state, {"A": variants[idx]["team_a"], "B": variants[idx]["team_b"]}, match.venue),
                }
